-- =====================================================
-- SCRIPT DE LIMPIEZA DE MIGRACIONES
...
1. 20250122000000-create-system-settings.sql
   - Crea tabla system_settings que no se usa en la app
   - Incluye funciones y políticas innecesarias

2. 20250123000000-fix-business-hours-format.sql
   - Corrige formato de horarios de system_settings
   - No se usa en la app actual

3. 20250123000001-fix-system-settings-policies.sql
   - Corrige políticas de system_settings
   - No se usa en la app actual

4. 20250123000002-make-all-settings-public.sql
   - Hace públicas todas las configuraciones
   - No se usa en la app actual

5. 20250124000000-configure-s3-storage.sql
   - Configuración de S3 que no se usa
   - La app usa Cloudinary para imágenes

6. 20250125000000-fix-products-rls-policies.sql
   - Corrige políticas de productos
   - Se puede consolidar con la migración inicial

7. 20250125000001-fix-categories-rls-policies.sql
   - Corrige políticas de categorías
   - Se puede consolidar con la migración inicial

8. 20250125000002-configure-storage-policies.sql
   - Configuración de storage innecesaria
   - La app usa Cloudinary

9. 20250125000003-fix-reservation-items-trigger.sql
   - Trigger innecesario para reservation_items
   - Se puede manejar en la aplicación

10. 20250126000000-add-reservation-delete-policies.sql
    - Políticas de eliminación de reservas
    - Se puede consolidar con otras políticas

11. 20250127000000-add-payment-proof-upload.sql
    - Funcionalidad de upload de comprobantes
    - No se usa en la app actual

12. 20250715124000_create_availabilities_table.sql
    - Tabla de disponibilidades que no se usa
...
8. 20250116000001-fix-user-roles-policies.sql
   - Políticas de roles (aunque no se use user_roles)
