-- Script para actualizar zonas - versión adaptativa

-- Primero verificar si las columnas existen y agregarlas si es necesario
DO $$ 
BEGIN
    -- Agregar columna postal_codes si no existe
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'zones' AND column_name = 'postal_codes') THEN
        ALTER TABLE public.zones ADD COLUMN postal_codes TEXT[];
    END IF;
    
    -- Agregar columna neighborhoods si no existe
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'zones' AND column_name = 'neighborhoods') THEN
        ALTER TABLE public.zones ADD COLUMN neighborhoods TEXT[];
    END IF;
END $$;
